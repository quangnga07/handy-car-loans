...
class Page_m extends CI_Model
{
	...
	*/
	...
	{
		$sql = "select * from cms_pages where page_type is null or page_type = '' ORDER BY id ASC";
		$query = $this->db->query($sql);
		return $query->result();
	}

	public function get_application_pages()
	{
		$this->db->where('page_type','application');
		return $this->db
			->get('cms_pages')
			->result();
	}
	...
	public function get_all_keywords()
		{
			...
			foreach($result as $row){
				if(isset($row->meta_keywords)){
					$keyword_string .=$row->meta_keywords;
				}
			}
			return $keyword_string;
		}
	public function process_keywords($keywords)
	{
	...
	public function create($input)
	{
		...
		$newdata  = array(
			...
			'meta_keywords' 	=> isset($input['meta_keywords']) ? $this->process_keywords($input['meta_keywords']) : '',
		...
		$id = $this->db->insert('cms_pages',$newdata);
...
public function edit($id,$input)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('cms_pages',$newdata);
		$this->db->trans_complete();

		$this->update_widget_instance_table($id,$input);
		return (bool) $this->db->trans_status();
	}

	public function update_widget_instance_table($id,$input,$area='right')
		{

			$area = @$input['area'];

			$this->db->where(array('page_id'=>$id,'area'=>$area));
			$this->db->delete('cms_widget_instances');

			$this->db->where(array('page_id'=>$id,'area'=>'middle'));
			$this->db->delete('cms_widget_instances');

			$rows_of_widgets = array();
			$counter = 0;
			$skip_no = $this->db->count_all('cms_widgets');

			if(isset($input['widget_id'])) {
				foreach(@$input['widget_id'] as $widget){
					if($counter>=$skip_no){
						$new_row = array();
						$new_row['page_id'] = $id;
						$new_row['widget_id'] = $widget;
						$new_row['widget_title'] = @$input['widget_title'][$counter];
						$new_row['widget_content'] = @$input['widget_content'][$counter];
						$new_row['widget_class'] = @$input['widget_class'][$counter];
						$new_row['area'] = $area;
						$new_row['order'] = $counter;
						array_push($rows_of_widgets,$new_row);
						$this->db->insert('cms_widget_instances',$new_row);
					}
					$counter = $counter + 1;
				}
			}
			...
			$counter = 0;
			foreach($input['middle_widget_id'] as $widget){
				if($counter>=$skip_no){
					$new_row = array();
					$new_row['page_id'] = $id;
					$new_row['widget_id'] = $widget;
					$new_row['widget_title'] = @$input['middle_widget_title'][$counter];
					$new_row['widget_content'] = @$input['middle_widget_content'][$counter];
					$new_row['widget_class'] = @$input['middle_widget_class'][$counter];
					$new_row['area'] = 'middle';
					$new_row['order'] = $counter;
					array_push($rows_of_widgets,$new_row);
					$this->db->insert('cms_widget_instances',$new_row);
			...
			//$id = $this->db->insert_batch('cms_widget_instances',$rows_of_widgets);
			return true;
