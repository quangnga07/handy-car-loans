...
(function( $ ){
		...
		upload: function(files){
			...
			if (typeof files !== "undefined") {
                for (var i=0; i< nfiles; i++) {
                	...
                	//---- customize to fix one by one upload
                	var fileId = [];
                	if( $(".files-to-upload ul").has('li') ) {
						$(".files-to-upload ul li").each(function(){	      
						    fileId = $(this).attr('id');
						    fileId = fileId.split('FILE');
						});
                	}
                	
                	if( fileId[1] != [] && fileId[1] != undefined ) {
                		num = Number(fileId[1]) + 1;
                	} else {
                		num = i
                	}
                	//----
					...
					var xload = {fileno:num, filename: files[i].name, ftype: ext, perc:0, sent: 0, total: 0};						
					settings.start(xload);			
					...
					nef.append("user_id", settings.user_id);
	...
	if(opt) $.extend(settings, opt);
	  ...
	  if ( !((window.location != window.parent.location) ? true : false) )
	  {
		...
		$(this).append('<div class="mf_upload_m"><form method="post" enctype="multipart/form-data" class="file" action="'+settings.post_upload+'" target="'+settings.iframe+'"><input type="file" class="file" name="'+settings.file_elem+'" id="'+settings.file_elem+'" multiple /><input type="hidden" name="folder" value="'+settings.folder+'" /><input type="hidden" name="id" value="'+settings.user_id+'"><input type="hidden" name="file_element" value="'+settings.file_elem+'" /></form><div class="mf_upload_ins"></div></div>');
        ...
        $('.mf_upload_ins').on('click', function(e) {
			var flag = 0;
			console.log(flag);
